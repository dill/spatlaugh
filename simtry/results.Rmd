...
for(i in seq_along(f_names)){
  ...
  rr <- reasonable_range(big_res$sp1)
  # build a data.frame to count clipped observation
  clipped_d <- ddply(big_res, .(names), clipped_obs, name="sp1", rr=rr)
  p <- ggplot(big_res)+
    geom_histogram(aes(sp1))+
    facet_wrap(~names, scales="free_x",nrow=2)+
    scale_x_continuous(limits = rr) +
    geom_text(aes(label=V1, x=rr[1]+(rr[2]-rr[1]), y=50 ), data=clipped_d) +
    ggtitle(paste(f_names[i], "sp1"))
  print(p) 
  
  rr <- reasonable_range(big_res$sp2)
  # build a data.frame to count clipped observation
  clipped_d <- ddply(big_res, .(names), clipped_obs, name="sp2", rr=rr)
  p <- ggplot(big_res)+
    geom_histogram(aes(sp2))+
    facet_wrap(~names, scales="free_x",nrow=2)+
    scale_x_continuous(limits = rr) +
    geom_text(aes(label=V1, x=rr[1]+(rr[2]-rr[1]), y=50 ), data=clipped_d) +
    ggtitle(paste(f_names[i], "sp2"))
  print(p) 
